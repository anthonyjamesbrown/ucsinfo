<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="BladeInventoryHist.aspx.vb" Inherits="UCS.BladeInventoryHist" EnableEventValidation="false" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <h2>
        UCS Blade Inventory History</h2>
    <table>
        <tr>
            <td class="style8">
                Data Center:</td>
    ...
    <asp:GridView ID="GridView1" runat="server" 
        ...
        <Columns>
            <asp:BoundField DataField="DateCollected" HeaderText="Date" 
                SortExpression="DateCollected" />
...
<PagerStyle CssClass="pgr"></PagerStyle>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ConnectionString="<%$ ConnectionStrings:UCSConnectionString %>"    
        SelectCommand="SELECT [DateCollected], [Ucs], [Name], [Model], [ChassisId], [SlotId], [Serial], [OperState], [WWNs], [BootPolicyName], [HostFwPolicyName], Substring(Replace([SPDN], 'org-root/org-', ''),1, 3) As SPDN FROM [v_Blades_ServiceProfile_Historical] WHERE ((([Ucs] LIKE '%' + @Ucs + '%') AND ([Ucs] Like @Ucs2)) AND (([Name] LIKE '%' + @Name + '%') OR ([WWNs] LIKE '%' + @WWNs + '%') OR ([Serial] LIKE '%' + @Serial + '%') OR ([Model] LIKE '%' + @Model + '%') OR ([BootPolicyName] LIKE '%' + @BootPolicyName + '%') OR ([HostFwPolicyName] LIKE '%' + @HostFwPolicyName + '%')))  ORDER BY [Ucs], [ChassisId], [SlotId] ASC">
        <SelectParameters>
            <asp:ControlParameter ControlID="DC_DropDownList" DefaultValue="Select" Name="Ucs" 
